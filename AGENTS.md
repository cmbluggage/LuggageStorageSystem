...
- **Flight number is its own column** (`bookings.flight_number`, migration 006), not stuffed into `notes`. The booking flow's "Arrival Flight Number (Optional)" field (`book/page.tsx`) posts `flightNumber` separately; `notes` is free text only. `BookingRecord.flightNumber` and `.notes` are both surfaced (and both editable via `BookingEditForm`) — don't collapse them back into one field.
- **Cash collection tracking**: `bookings.cash_collected_by` (→ `staff.user_id`) / `cash_collected_at` (migration 006) record who physically collected a cash payment and when — distinct from `payment_status` just flipping to `'paid'`. Set via `markCashCollected()` in `db.ts` / `PATCH /api/staff/bookings/[id]/collect-cash`, surfaced through `CashCollectButton` (`src/components/admin/CashCollectButton.tsx`, shared by `/staff` and `/admin`). Cash-only — card bookings settle automatically and never go through this path. `cashCollectedByName` is stripped from every customer-facing response (`/api/bookings*`) alongside `passportNo` — it's internal staff identity, not the customer's business.

## Database Schema (Supabase)
Migrations live in `supabase/migrations/` (`001_schema.sql`, `002_add_insurance_hourly_slots.sql`, `003_public_inserts.sql`, `004_security_settings_ops.sql`, `005_item_tier_images.sql`, `006_flight_number_cash_tracking_and_airport_fee_cleanup.sql`); seed data in `supabase/seed.sql`. Ad-hoc RLS/auth patches: `supabase/fix_admin_rls.sql`, `supabase/fix_auth_passwords.sql`. `src/lib/supabase/types.ts` is the authoritative TS shape. Migrations are append-only history — a later migration undoes an earlier one (006 drops what 001/002/004 added) rather than editing those files in place.

- `public.locations`: Dropoff/pickup points. Flags: `is_airport`, `requires_stripe`, `allows_cash`, plus `dropoff_surcharge_usd` / `pickup_surcharge_usd` — the only airport-cost lever, see Business Rules above.
...
- `public.bookings`: Main reservation table — FKs to `customers`/`locations`, computed totals, `payment_method` (`cash`|`stripe_simulated`), `payment_status`, `booking_status`, `qr_code_token`, `duration_days`, `insurance_enabled`, `idempotency_key`, `flight_number`, `cash_collected_by`/`cash_collected_at` (migration 006).
...
- `public.staff`: Portal users — `role` (`staff`|`superadmin`) linked to Supabase Auth `user_id`. `bookings.cash_collected_by` references `staff.user_id` (not `auth.users.id` directly) so PostgREST can embed the collecting staff member's name in one query.
...
1. Run `supabase db push` or execute `supabase/migrations/001_schema.sql`, `002_add_insurance_hourly_slots.sql`, `003_public_inserts.sql`, `004_security_settings_ops.sql`, `005_item_tier_images.sql`, `006_flight_number_cash_tracking_and_airport_fee_cleanup.sql` (in order) and `supabase/seed.sql` in your Supabase SQL Editor.
